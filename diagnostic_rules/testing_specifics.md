...
> **注意**：具体性能指标的基准值（Baseline）应以 `documentation_specifics.md` 中 PICS 文档声明的参数（如 P2, P2*）为准。

1. **UDS 响应时间**
   - **单次请求往返时间**：在无总线拥堵场景下，应符合 PICS 声明的 P2 时间要求（通常 < 50ms 或 < 200ms，视 ECU 而定）。
   - **大负载传输**：读写 Block 数据时的吞吐量应记录实测值，并与 OEM 要求比对。

2. **全车扫描时间**
   - **指标定义**：从点击“全车扫描”到所有 ECU 的 DTC 结果展示完毕的总耗时。
   - **基准**：应在指定车型上测量并作为版本发布的基准指标。
